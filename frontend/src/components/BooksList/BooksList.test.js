...
describe("BookList", () => {
  ...
  it("should redirect to orderPlaced screen with isOrderPlaced true",()=>{
    ...
    expect(mockHistoryPush).toHaveBeenCalledWith("/orderConfirmation", {"book": {"id": 1, "name": "Book 1", "price": 100}, isOrderPlaced: true});
  });
  ...
  it("should redirect to orderPlaced screen with isOrderPlaced false",()=>{
    ...
    fireEvent.click(linkElement);
    expect(mockHistoryPush).toHaveBeenCalledTimes(1);
    expect(mockHistoryPush).toHaveBeenCalledWith("/orderConfirmation", {"book": {"id": 1, "name": "Book 1", "price": 100}, isOrderPlaced: false});
  });
});
